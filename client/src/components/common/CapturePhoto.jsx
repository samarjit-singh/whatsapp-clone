...
function CapturePhoto({ hide, setImage }) {
  ...
  useEffect(() => {
    ...
    const startCamera = async () => {
      ...
      videoRef.current.srcObject = stream; // setting live video feed
    };
    startCamera();
    return () => {
      // return stop camera from recording when camera is unmounted
  ...
  const capturePhoto = () => {
    ...
    setImage(canvas.toDataURL("image/jpeg")); // base64
    hide(false);
  };
